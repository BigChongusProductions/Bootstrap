...
5. Check canonical templates: does `~/.claude/dev-framework/templates/` exist?

6. Present a compact status report:
...
Engine:
  ...
  Scripts:         [X/11 deployed]
  Frameworks:      [X/9 deployed]
  Hooks:           [X/12 deployed / missing]
  Agents:          [X/2 deployed / missing]
  Rules:           [X/3+ deployed / missing]
  Settings:        [deployed / missing]
  Git hooks:       [installed / missing]
  Tracking files:  [X/5 deployed]
  Templates:       [installed / missing]

Next step: [what to do based on current state]
```

7. Based on the state, recommend next action:
   ...
   - No canonical templates → "Run /setup-templates to extract canonical templates"
