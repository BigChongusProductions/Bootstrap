...
verify_core_files() {
  ...
  if [ "$LIFECYCLE" = "full" ]; then
    ...
    # Full lifecycle creates spec templates
    if [ -d "$DIR/specs" ]; then
      local SPEC_COUNT
      ...
      if [ "$SPEC_COUNT" -ge 1 ]; then
        ...
        fail "specs/ directory is empty (expected spec templates)"
      fi
    fi
  fi
}
...
verify_scripts() {
  ...
  # New step-26 scripts
  for s in save_session.sh shared_signal.sh harvest.sh db_queries_legacy.sh; do
    if [ -f "$DIR/$s" ]; then
...
verify_lifecycle_specifics() {
  ...
  if [ "$LIFECYCLE" = "full" ]; then
    ...
    # Check legacy script has full phase case
    if [ -f "$DIR/db_queries_legacy.sh" ]; then
      chk_contains "Legacy script has P1-ENVISION case" "$DIR/db_queries_legacy.sh" "P1-ENVISION"
      chk_contains "Legacy script has P9-EVOLVE case" "$DIR/db_queries_legacy.sh" "P9-EVOLVE"
    fi
    ...
    if [ -f "$DIR/db_queries_legacy.sh" ]; then
      chk_contains "Legacy script has P1-PLAN case" "$DIR/db_queries_legacy.sh" "P1-PLAN"
      chk_contains "Legacy script has P3-SHIP case" "$DIR/db_queries_legacy.sh" "P3-SHIP"
    fi
  fi
}
