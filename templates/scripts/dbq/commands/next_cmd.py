...
def _show_forward_smart(db: Database, config: ProjectConfig):
    ...
    rows = db.fetch_all(
        ...
        f"( "
        f"  (6 - CASE t.phase {phase_case} ELSE 7 END) * 100 "
...
def _show_blocked(db: Database):
    ...
    rows = db.fetch_all(
        ...
        "b.id AS blocker_id, b.assignee AS blocker_assignee, "
        "b.status AS blocker_status, t.phase "
        "FROM tasks t "
        "JOIN tasks b ON t.blocked_by = b.id "
        "WHERE t.status='TODO' AND t.assignee='CLAUDE' "
        "AND t.queue != 'INBOX' "
    ...
    for r in rows:
        ...
        print(f"    {r['id']}{lb_tag}  {r['title']}  "
              f"← {r['blocker_id']} ({r['blocker_assignee']}, {r['blocker_status']})")


def _show_stale_blockers(db: Database):
    """Show tasks blocked by nonexistent task IDs."""
    stale_count = db.fetch_scalar(
        "SELECT COUNT(*) FROM tasks t "
    ...
    if stale_count > 0:
        ...
        for r in rows:
            print(f"    {r['id']}  {r['title']}  ← {r['blocked_by']} (NOT FOUND)")
