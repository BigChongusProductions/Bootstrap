...
if ! command -v sqlite3 &>/dev/null; then
    echo -e "${RED}❌ PREREQUISITE FAILED: sqlite3 is not installed${RESET}"
    echo "   Install: brew install sqlite3 (macOS) or apt install sqlite3 (Linux)"
    exit 2
fi
...
FWD_TOTAL=$(sqlite3 "$DB" "SELECT COUNT(*) FROM tasks WHERE phase='$PHASE' AND COALESCE(track,'forward')='forward';")
FWD_DONE=$(sqlite3 "$DB" "SELECT COUNT(*) FROM tasks WHERE phase='$PHASE' AND COALESCE(track,'forward')='forward' AND status='DONE';")
FWD_INCOMPLETE=$(sqlite3 "$DB" "
