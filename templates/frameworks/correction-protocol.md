...
**Enforcement:** `bash ~/.claude/harvest.sh --bootstrap` scans all projects for unescalated template-affecting lessons.

**Consumption:** Backlog items are reviewed and applied to templates via `bash ~/.claude/dev-framework/apply_backlog.sh`. During `/activate-engine`, pending P0/P1 items are surfaced as advisories.
